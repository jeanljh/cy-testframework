...
describe('Car search test suite', () => {
    ...
    it('Test select driver age', () => {
        ...
        carSearch.txtTooltip()
        // .as('tooltip')
        .should('have.text', 'Additional Fee may apply for driver under 30 Yrs or above 65 Yrs old, at the time of rental. Please check term and conditions on payment page.')
        .and('not.be.visible')
        .parent()
    ...
    it('Test get vendor type', () => {
        ...
        carSearch.ddlVendorCode()
        .find(':selected')
        .should('have.text', 'Car Company')
        carSearch.ddlVendorCode()
        .find('option')
    ...
    it('Test get vehicle type', () => {
        ...
        carSearch.tabCar().click()
        carSearch.lnkAdvOption().click({force: true})
        carSearch.ddlVehicleType()
        .find(':selected')
        .should('have.text', 'Car Type')
        carSearch.ddlVehicleType()
        .find('option')
        .should('have.length.gt', 0)
    ...
    it('Test vendor code options', () => {
        const arrRes = []
        carSearch.ddlVendorCode().find('option').each(e => arrRes.push(e.text()))
        cy.wrap(arrRes).should('deep.equal', data.vendorCode)
    })

    it('Test vehicle type options', () => {
        const arrRes = []
        carSearch.ddlVehicleType().find('option').each(e => cy.wrap(e).invoke('text').then(t => arrRes.push(t)))
